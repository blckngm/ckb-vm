...
use super::context::{ExecutionContext, WithCyclesFunc, WithDebugger, WithSyscall};
...
impl<Inner: SupportMachine, S: ExecutionContext<Inner>> SupportMachine
    for DefaultMachine<Inner, S>
{
    fn cycles(&self) -> u64 {
        self.inner.cycles()
    }
...
impl<Inner: SupportMachine, S: ExecutionContext<Inner>> Machine for DefaultMachine<Inner, S> {
    fn ecall(&mut self) -> Result<(), Error> {
...
/// if you want to use simple callbacks to customize the context.
...
impl<Inner, Ctx> DefaultMachineBuilder<Inner, Ctx> {
    /// Add a syscall handling callback.
    pub fn syscall<F>(self, syscall: F) -> DefaultMachineBuilder<Inner, WithSyscall<Ctx, F>>
    where
        // Not strictly necessary. For better type inference.
